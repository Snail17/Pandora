...
public class MPermissionUtils {
     ...
     * 检查所传递对象的正确性
     *
     * @param object object 必须为activity orfragmen
     */
    ...
    public static int hasPermissions(Context context, String permissions) {
        int broadcastNoteOp = 0;
        if (isOverMarshmallow()) {
            String broadcastToOp = AppOpsManagerCompat.permissionToOp(permissions);
            broadcastNoteOp = TextUtils.isEmpty(broadcastToOp) ? -1 : AppOpsManagerCompat.noteOp(context, broadcastToOp, Process.myUid(), context.getPackageName());
        }
        return broadcastNoteOp;
    }
